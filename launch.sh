...
export PATH="$PAK_DIR/bin/aarch64:$PAK_DIR/bin/$PLATFORM:$PAK_DIR/bin/shared:$PAK_DIR/bin:$PATH"

SERVICE_NAME="memcheat"
HUMAN_READABLE_NAME="Memory Editor"
NETWORK_PORT=8080
NETWORK_SCHEME="http"
...
main() {
    ...
    chmod +x "$PAK_DIR/bin/aarch64/$SERVICE_NAME" 2>/dev/null || true
    chmod +x "$PAK_DIR/bin/$PLATFORM/jq"
    chmod +x "$PAK_DIR/bin/$PLATFORM/minui-list"
    chmod +x "$PAK_DIR/bin/$PLATFORM/minui-presenter"
    chmod +x "$PAK_DIR/bin/shared/service-on"
    chmod +x "$PAK_DIR/bin/shared/service-off"
    chmod +x "$PAK_DIR/bin/shared/service-is-running"
    chmod +x "$PAK_DIR/bin/shared/on-boot"
